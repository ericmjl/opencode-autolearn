...
def _call(method, path, pdir, body=b""):
...
def test_overview(tmp_path):
    ...
    reg.add("a memory")
    status, body = _decode(_call("GET", "/api/overview", p))
    assert status == 200
    assert body["total"] == 1
...
def test_memories_list_and_detail(tmp_path):
    ...
    status, rows = _decode(_call("GET", "/api/memories", p))
    assert status == 200 and len(rows) == 1
    status, detail = _decode(_call("GET", f"/api/memory/{rec['id']}", p))
...
def test_index_html_served(tmp_path):
    p = _persona(tmp_path)
    status, body = _decode(_call("GET", "/", p))
...
def test_strengthen_post(tmp_path):
    ...
    status, body = _decode(_call("POST", f"/api/memory/{rec['id']}/strengthen", p))
...
def test_candidate_dismiss(tmp_path):
    ...
    status, body = _decode(_call("POST", "/api/candidate/c1/dismiss", p))
    assert status == 200 and body["ok"] is True
